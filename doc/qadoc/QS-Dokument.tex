...
\begin{document}
	...
	\chapter{Qualitäsziele}
		...
		Begründung: Framework soll zur Übermittlung von sicherheitskritischen Daten dienen. Soll in anderen Programmen genutzt werden. Nutzer "vertraut" dem Framework.
		
		Maßnahmen: Die Zuverlässigkeit kann durch Testen verbessert werden. Deshalb benutzen wir die boost.test Bibliothek, die automatische Tests in C++ ermöglicht. Für jede Methode wird mindestens ein Test geschrieben und für jede Aufgabe (siehe Testbarkeit) mindestens zwei Tests. Außerdem führen wir Code Reviews durch.

		Prozess: Diese werden mindestens einmal pro Woche auf der aktuellen Version der Software komplett ausgeführt. Fehler werden im Ticketsystem unseres SCM-Servers eingetragen.
		
		 Nach Abschluss einer User Story wird der Code von einem an diesem Use Case unbeteiligten Teammitglied anhand einer Checkliste überprüft. Mögliche Fehler werden schnellstmöglich von den Entwicklern behoben. Dann wird der Vorgang wiederholt, bis die Kriterien erfüllt sind.
		
        	\section{Wartbarkeit}
		Begründung: Open Source, soll erweitert werden
		...
		vielleicht als drittes Ziel??
		Begründung: wie Wartbarkeit

		Maßnahme: Dokumentation

		Prozess: automatisch generiert, dann vervollständigt
